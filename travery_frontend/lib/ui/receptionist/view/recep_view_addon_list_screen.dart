...
class RecepViewAddonListScreen extends StatefulWidget {
  ...
  @override
  State<RecepViewAddonListScreen> createState() =>
      _RecepViewAddonListScreenState();
}

class _RecepViewAddonListScreenState extends State<RecepViewAddonListScreen> {
  String selectedFilter = 'TẤT CẢ';

  final List<String> filters = ['TẤT CẢ', 'SPA', 'GIẶT ỦI', 'ĂN UỐNG'];
  ...
  void _onResult() {
    ...
    if (widget.viewModel.loadAddOnOrders.error) {
      ...
      if (result is core.Error) {
        errorMessage = (result as core.Error).error.toString().replaceAll(
          'HttpException: ',
          '',
        );
      }
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, child) {
        final isLoading =
            widget.viewModel.loadAddOnOrders.running ||
        ...
        final filteredOrders = allOrders.where((order) {
          ...
          if (selectedFilter == 'SPA' && order.category.toUpperCase() == 'SPA')
            return true;
          if (selectedFilter == 'GIẶT ỦI' &&
              order.category.toUpperCase() == 'LAUNDRY')
            return true;
          if (selectedFilter == 'ĂN UỐNG' &&
              order.category.toUpperCase() == 'FOOD')
            return true;
          return false;
        }).toList();
                        ...
                        return Padding(
                          ...
                          child: RecepAddOnCard(
                            ...
                            onStatusUpdate: (id, status) {
                              widget.viewModel.updateOrderStatus.execute((
                                id,
                                status,
                              ));
                            },
