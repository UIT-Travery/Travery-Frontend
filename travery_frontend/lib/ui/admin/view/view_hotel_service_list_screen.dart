...
class _ViewHotelServiceListScreenState
  ...
  void initState() {
    ...
    widget.viewModel.deleteService.addListener(_onDeleteResult);
  ...
  void didUpdateWidget(ViewHotelServiceListScreen oldWidget) {
    ...
    if (oldWidget.viewModel != widget.viewModel) {
      ...
      oldWidget.viewModel.deleteService.removeListener(_onDeleteResult);
      widget.viewModel.loadServices.addListener(_onLoadResult);
      widget.viewModel.deleteService.addListener(_onDeleteResult);
      widget.viewModel.loadServices.execute(widget.hotelId);
    }
  ...
  void dispose() {
    ...
    widget.viewModel.deleteService.removeListener(_onDeleteResult);
    super.dispose();
  }
  ...
  void _onDeleteResult() {
    final cmd = widget.viewModel.deleteService;
    if (cmd.error) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(cmd.error.toString()),
          backgroundColor: Colors.red,
        ),
      );
                                  ...
                                  if (value == 'edit') {
                                    await context.push(
                                      ...
                                      extra: {
                                        ...
                                        'serviceId': svc.id,
                                        'serviceType': svc.category.displayName,
                                    ...
                                    widget.viewModel.loadServices.execute(
                                      widget.hotelId,
                                    );
                                    ...
                                    showDialog(
                                      ...
                                      builder: (context) => AlertDialog(
                                        ...
                                        content: Text(
                                          'Bạn có chắc chắn muốn xóa dịch vụ "${svc.name}" không?',
                                        ),
                                        actions: [
                                          TextButton(
                                            onPressed: () =>
                                          ...
                                          TextButton(
                                            onPressed: () {
                                              Navigator.pop(context);
                                              widget.viewModel.deleteService
                                                  .execute(svc.id);
                                            },
                                            child: const Text(
                                              'Xóa',
