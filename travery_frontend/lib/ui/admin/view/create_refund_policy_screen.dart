...
import 'package:travery_frontend/ui/admin/view_model/create_refund_policy_view_model.dart';
import 'widgets/dropdown_button.dart';
import 'widgets/input_text_field.dart';
import 'widgets/large_button.dart';

class CreateRefundPolicyScreen extends StatefulWidget {
  final CreateRefundPolicyViewModel viewModel;

  const CreateRefundPolicyScreen({super.key, required this.viewModel});

  @override
  State<CreateRefundPolicyScreen> createState() => _CreateRefundPolicyScreenState();
}

class _CreateRefundPolicyScreenState extends State<CreateRefundPolicyScreen> {
  final _nameController = TextEditingController();
  String _selectedServiceType = 'HOTEL';
  ...
  void initState() {
    super.initState();
    widget.viewModel.createPolicy.addListener(_onResult);
  }

  @override
  void didUpdateWidget(CreateRefundPolicyScreen oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.viewModel != widget.viewModel) {
      oldWidget.viewModel.createPolicy.removeListener(_onResult);
      widget.viewModel.createPolicy.addListener(_onResult);
    }
  }
  ...
  @override
  void dispose() {
    widget.viewModel.createPolicy.removeListener(_onResult);
    _nameController.dispose();
    super.dispose();
  }

  void _onResult() {
    final cmd = widget.viewModel.createPolicy;
    if (cmd.completed) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Đã tạo chính sách hoàn tiền'),
  ...
  void _submit() {
    ...
    widget.viewModel.createPolicy.execute(RefundPolicyRequest(
      name: name,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            ListenableBuilder(
              listenable: widget.viewModel.createPolicy,
              builder: (context, _) {
                final running = widget.viewModel.createPolicy.running;
                return LargeButton(
